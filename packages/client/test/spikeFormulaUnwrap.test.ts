/**
 * SPIKE (block-object-chrome) — Mechanic B: formulas "unwrap to plain text" instead of deleting. Backspace
 * at the right edge (already shipped: unwrapFormulaBackspace) AND the new symmetric forward-Delete at the
 * left edge replace the rendered formula with its plain-text SOURCE (the spec), caret left in that text.
 * Re-triggering the existing auto-detect re-renders it. Purely additive to buildFormulaPlugins.
 */
