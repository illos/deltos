...
describe('EditorControlStrip — desktop flat (expanded)', () => {
  it('renders ALL tools inline up front (no click-to-expand) + Undo/Redo', async () => {
    ...
    // The bottom context slot is OUT OF FLOW + only rendered when populated → absent when idle (no jump).
    expect(document.querySelector('.editor__context-slot')).toBeNull();
  });
