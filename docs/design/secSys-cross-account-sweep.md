...
# Account-identity security surfaces → moved

The D6-expanded account-identity pressure-test (accountId + usernames: directory-vs-ownership separation, username namespace integrity, recovery/QR re-association, add/replace-credential authz + lockout guard, accountId properties, the accountFingerprint→accountId migration safety window, the load-bearing per-query accountId scope, and the ADDITIVE-no-reopen confirm) now lives in the parity-named sibling **`secSys-account-identity-review.md`** (beside devSys's `account-identity-strawman.md`). This sweep doc remains the data-layer cross-account finding + load-bearing-control reference it builds on.
