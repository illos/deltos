...
>   - 🚨 **2FA incident (live-review caught it):** enabling 2FA bricked login — server never emitted `totp_required` so the login UI had no code step. **Bug-1 FIXED + DEPLOYED LIVE** (worker `512fa17f`); login = username+password+6-digit code. **Bug-2 root cause REVISED (Jim spotted it): recovery RESET WORKS end-to-end** (confirmed live — Jim reset acct `WDe3ao…` + logged in with the new password; matches pilot's fresh-account proof). The real bug is a small **CLIENT UX gap: /reset succeeds but the screen shows "connect error" / no success state** (the post-reset /finalize 503 is surfaced as overall failure, masking success). devSys verdict: the post-reset /finalize 503 is **EXPECTED** (reset mints no session by design; F13 fail-closed on the no-auth finalize call) — NOT a server gap. Fix is **purely CLIENT (#51, gruntSys):** stop calling /finalize after /reset → route to /login → fresh login re-establishes the durable cookie. Deploy to live → Jim reset-screen re-test. Username-UNIQUENESS code review → secSys (UNIQUE constraint + atomic claimUsername + could-0003-dup; fold+upgrade #50 only if a dup is code-possible, else close). The earlier 'stale binding' was real but regenerate fixed it (reset then 200'd); 'still broken' was purely the missing success screen. Hardening backlog #50 (re-establish-recovery on accountId re-key) = LOW. See [[2fa-login-incident-bugs]] + [[recovery-phrase-accountid-binding-fragility]].
>   - 🗑️ **Data/accounts DISPOSABLE until real users** (Jim, in `CLAUDE.md` + [[pre-real-users-clean-state-bias]]). Verified-safe clean-slate D1 wipe authorized as NON-BLOCKING hygiene (preserve migration table; clears stale accounts) — must not block history.
> - ⏪ **NOTE HISTORY + UNDO/REDO — built, deployed, then ROLLED BACK 2026-06-20** (live reverted to pre-history `512fa17f`) after Jim hit "notes vanish on edit+sync." **Root cause = NOT history (exonerated by devSys code-proof: #45/#46 never write notebookId).** Actual bug PRE-dates history, live on `512fa17f` too: a **stale `notebookPointer`** migrates a legacy per-device RANDOM default id from localStorage as `currentNotebookId` → notes land on a non-canonical 'Notes' id → the CORRECT #16/#22 server reassign (orphaned → canonical default) moves them on edit+sync → they leave the stale-id view = "vanish." The 3 'Notes' notebooks = `isDefault=0` RESIDUE (unique index guarantees ≤1 real default). Custom notebooks (e.g. 'books') unaffected — consistent synced id. Connects to [[sync-notebookid-per-device-regression]] (per-device-id class, default path still leaked).
>   - **FIX (approved):** client **reconcile-stale-pointer** (adopt canonical default, drop legacy random id) + gate test **create→EDIT→sync→STAYS-PUT** + WIPE-now for Jim (disposable) + **0010 CONSOLIDATE** for real users. History **re-lands WITH the notebook fix in ONE integrated deploy**, hard-gated on the stays-put test. secSys #47 PASS + #25/#40 belt still good. NON-BLOCKING restore copy nit → gruntSys.
>   - **Process win:** "vanishing on the newest deploy" looked like a history regression (recency bias) but the fork test (repro on pre-history build) EXONERATED it — confirm against the pre-deploy build before blaming the new feature ([[prod-diagnosis-confirm-actual-failure]] class).
>   - 🚨 **DEEPER ROOT (Jim, screenshot + fresh-account test): the CLIENT LOCAL STORE IS NOT ACCOUNT-ISOLATED.** A brand-new account shows 4 'Notes' notebooks + the 19-note notebook + 'Book'-with-notes — ALL from PRIOR accounts. Notes AND notebooks PERSIST ACROSS ACCOUNTS on the same browser because the local read path (notes list + notebook switcher + the localStorage notebookPointer) doesn't filter by the current authenticated accountId. The "duplicate defaults / notes moving" were SYMPTOMS of every account's data coexisting in one unscoped local store. Server is account-scoped (audited); the gap is the CLIENT. **This subsumes the single-default + stale-pointer fixes.** Fix = scope/partition the local store by current accountId (set on login/switch, cleared/filtered on account change). Gate: login A → create notes → login B → B shows ZERO of A's data. **CONFIRMED by devSys** (@6e8b43f analysis); canonical tracking = [[client-account-isolation-gap]] (#52, audit map `docs/design/client-account-isolation-audit.md`). Per Jim's directive: **thorough AUDIT → COMPREHENSIVE fix, never a quick patch**; the architectural choice (PARTITION-by-accountId vs CLEAR-on-switch vs FILTER-every-read) is a planSys+Jim DESIGN DECISION brought up BEFORE build. @6e8b43f within-account reconcile = PARKED (symptom-only).
>   - ✅ **DECISION (Jim+navSys 2026-06-20): OPTION B — CLEAR-on-account-change + Jim's refinement FULL WIPE ON LOGOUT** (no orphaned local data after sign-out). Clear entire local store (Dexie notes/notebooks/versions/trash/queue + notebookPointer + caches) on BOTH logout AND login-as-different-account; best-effort flush-queue-if-online on logout (secondary). Rejected A (filter, leakiest); C (per-account DB) deferred — offline same-device multi-account NOT a v1 goal. Closes all 3 leak classes (read/write-migration/cold-boot), preserves render-before-auth same-account, self-purges pre-fix residue. devSys prototyping B; gate = login-A→B-sees-ZERO + no-A-pushes-under-B + no cold-boot-flash + logout→local-EMPTY. Audit `docs/specs/client-account-isolation-audit.md`. **✅ DEPLOYED LIVE (ver 0793931a) + USER-VERIFIED on a clean-slate DB (2026-06-20): full pass green — create/login/logout, 2FA setup+login, reset w/ + w/o 2FA, new+edit note (no vanishing), sign-out/in persist, and A→B→A shows NO cross-account notes (isolation confirmed). Incident CLOSED.** Server DB wiped clean (Jim ran it; agent can't — [[wrangler-d1-prod-route-to-user]]).
>   - 🧹 **POST-FIX FULL WIPE (Jim directive):** once the isolation fix deploys to live, wipe ALL accounts+notes+notebooks on a clean slate so no pre-fix pollution causes phantom weirdness. MUST hit BOTH layers — SERVER D1 (script @81bfa6e, preserve migrations) AND the CLIENT LOCAL store (Dexie + notebookPointer; a one-time local purge, since a server-only wipe leaves the polluted local store). Sequence: fix deploys → full wipe → Jim re-registers fresh.
>   - ⚠️ **UI-refresh foundation rode along INERT:** `2fad87d` inline-SVG icons (UI-refresh Lane 1) shipped but NOT wired (0B added, changes nothing live). The visual UI-refresh still owes a look/feel design pass WITH Jim before anything wires live — keep it inert until then.
